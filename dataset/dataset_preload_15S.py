...
class My_Dataset_json_preload_classMerge(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):   
        ...
        imgNO=self.imgNO_dataset[idx] # 241213 15S用。 21P、22G还是上面注释的2维读取
        # ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ 
        #label
...
# 15Stanford以外的dataset都由我单独save-npy文件，再通过该函数读取
# eeg_dataset,label_dataset = load_npy_dataset(
#     eeg_npy_dir, label_npy_dir,
#     subject_list, )
def load_npy_dataset(
        eeg_npy_dir, label_npy_dir, imgNO_npy_dir,
        subject_list, ):
    eeg_npy_fname_list=[]
    label_npy_fname_list=[]
    imgNO_npy_fname_list=[]
    for subject_str in subject_list:
        eeg_npy_fname='eeg_dataset_'+subject_str+'.npy'
        eeg_npy_fname_list.append(eeg_npy_fname)
        label_npy_fname='label_dataset_'+subject_str+'.npy'
        label_npy_fname_list.append(label_npy_fname)
        imgNO_npy_fname='imgNO_dataset_'+subject_str+'.npy'
        imgNO_npy_fname_list.append(imgNO_npy_fname)
    
    assert len(eeg_npy_fname_list)>0,'没有匹配的eeg_npy_fname'
    assert len(label_npy_fname_list)>0,'没有匹配的label_npy_fname'
    assert len(imgNO_npy_fname_list)>0,'没有匹配的imgNO_npy_fname_list'

    eeg_dataset = np.load(os.path.join(eeg_npy_dir,eeg_npy_fname_list[0]))
    if len(eeg_npy_fname_list)>1:
        for fname in eeg_npy_fname_list[1:]:
            eeg_dataset_this_sub = np.load(os.path.join(eeg_npy_dir,fname))
            eeg_dataset = np.concatenate([eeg_dataset, eeg_dataset_this_sub],axis=0)

    label_dataset = np.load(os.path.join(label_npy_dir,label_npy_fname_list[0]))
    if len(label_npy_fname_list)>1:
        for fname in label_npy_fname_list[1:]:
            label_dataset_this_sub = np.load(os.path.join(label_npy_dir,fname))
            label_dataset = np.concatenate([label_dataset, label_dataset_this_sub],axis=0)

    imgNO_dataset = np.load(os.path.join(imgNO_npy_dir,imgNO_npy_fname_list[0]))
    if len(imgNO_npy_fname_list)>1:
        for fname in imgNO_npy_fname_list[1:]:
            imgNO_dataset_this_sub = np.load(os.path.join(imgNO_npy_dir,fname))

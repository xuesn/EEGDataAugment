...
# 区别只在于visual 加了linear头  用linear_out进行计算
def get_loss_acc_two_visual_linear(rank, world_size, 
    ...
    if loss_spc_weight > 0 or loss_triplet_weight > 0:
        loss_spc,loss_triplet = get_loss_contrast(rank, world_size, 
            emb_norm, label_classNO,
            criterion_spc, criterion_triplet)
        if loss_spc>999 or torch.isnan(loss_spc):
            loss_spc = torch.sum(0*emb_norm)
        if loss_triplet>999 or torch.isnan(loss_triplet):
            loss_triplet = torch.sum(0*emb_norm)
    ...
    if loss_vis_huber_weight > 0 :
        ...
        if loss_vis_huber_list[-1]>9999 or torch.isnan(loss_vis_huber_list[-1]):
            loss_vis_huber_list[-1]= torch.sum(0*linear_out )
        if layNO == 0:
    ...
    if loss_vis_infonce_weight > 0 :
        ...
        if loss_vis_infonce_list[-1]>9999 or torch.isnan(loss_vis_infonce_list[-1]):
            loss_vis_infonce_list[-1]= torch.sum(0*linear_out )
        if layNO == 0:
    ...
    if loss_vis_kl_weight > 0 :
        ...
        if loss_vis_kl_list[-1]>9999 or torch.isnan(loss_vis_kl_list[-1]):
            loss_vis_kl_list[-1]= torch.sum(0*linear_out )
        if layNO == 0:
...
def get_loss_acc_ce(rank, world_size, 
                        emb_norm, prob, 
                        eeg, label_onehot, 
                        criterion_ce,   ):
    ##### 一、
    # ce-loss 准确率 confusion-mat
    loss_ce = get_loss_ce(prob, label_onehot,criterion_ce)
    (right_num, accuracy_batch, 
        confusion_mat_batch) = get_acc_confusion_mat(prob, label_onehot)

    unused_parameter_tensor= 0* prob.sum() + 0* emb_norm.sum()  #230915新加
    # !!!

    loss_total = (loss_ce + unused_parameter_tensor)
    # unused_parameter_tensor

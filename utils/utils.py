...
# main_dir = '/data/snxue/' # 98 240 data 路径
# main_dir = '/data1/snxue/' # 98 data 路径
...
# augment
def get_short_augment_str(split_str):
    short_split_str = split_str
    if 'ch_symmetry' in split_str:
        short_split_str = split_str.replace('ch_symmetry','cs')

    elif 'amp_flip' in split_str:
        short_split_str = split_str.replace('amp_flip','af')

    elif 'time_flip' in split_str:
        short_split_str = split_str.replace('time_flip','tf')

    elif 'shift' in split_str:
        short_split_str = split_str.replace('shift','s')

    elif 'noise' in split_str:
        short_split_str = split_str.replace('noise','n')

    elif 'mask' in split_str:
        short_split_str = split_str.replace('mask','m')

    elif 'origin' in split_str:
        short_split_str = 'ori'

    return short_split_str
def get_dataset_str_for_save(data_dir_list,dataset_str):
    if 'Germany' in dataset_str:
        dataset_str_for_save = '22G'
    elif 'Purdue' in dataset_str:
        dataset_str_for_save = '21P'
    elif 'Stanford' in dataset_str:
        if '6' in dataset_str:
            dataset_str_for_save = '15S6'        
        elif '72' in dataset_str:
            dataset_str_for_save = '15S72'        

    suffix_list = []
    for data_dir in data_dir_list:
        if ('origin-10fold' in data_dir) or ('origin-9fold' in data_dir):
            split_str = str(1)# origin 未叠加平均 的 用1作为标识
            if '_' in data_dir:
                split_str = data_dir.split('fold_')[-1]
                split_str = get_short_augment_str(split_str)
                split_str = str(1)+'-'+split_str 

        elif 'mean' in data_dir:
            split_str = data_dir.split('mean')[-1]
            if '_' in data_dir:
                mean_str = split_str.split('_')[0]
                split_str = data_dir.split('_')[-1]
                split_str = get_short_augment_str(split_str)
                split_str = mean_str+'-'+split_str # origin 未叠加平均 的 用1作为标识

        else:
            assert False,'不识别的数据集'
        print(split_str)
        assert not split_str == '', 'Error split_str'
        suffix_list.append(split_str) 

    suffix_list.sort() # 让叠加平均小的在前面
    for suffix_str in suffix_list:
        suffix_str=str(suffix_str)
        dataset_str_for_save = dataset_str_for_save+'-'+str(suffix_str)
    return dataset_str_for_save
...
def get_model_encoder(model_str, dataset_str, 
    ...
    #     model = EEGNetv4_Encoder()
        ...
        # num_latents是MLP的参数，这里不用
    elif model_str == 'SCT':  # 22AG不pool性能好
...
from dataset.dataset_22Germany_nopreload import *
...
def choose_dataset(dataset_str):
    ...
    elif dataset_str in ['22Germany_train','22Germany_mean']:
        func_dataset = dataset_22Germany_mean_train
    elif dataset_str in ['22Germany_test']:
        func_dataset = dataset_22Germany_test
    elif dataset_str in ['22Australia_train','22Australia_test','22Australia_mean']:
